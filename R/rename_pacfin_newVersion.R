...
#' Used with updated version of PacFIN.Utilities 0.0.1.9999
#' as of Feb. 2021
#'
...
#' @return A data frame 
#'
#' @author Chantel Wetzel, Brian Langseth
#' @export
#'
rename_pacfin_newVersion <- function(data, area_grouping = NULL, area_names = NULL, fleet_grouping = NULL, fleet_names = NULL, fleet_column_name = "COND"){

	data <- PacFIN.Utilities::cleanPacFIN(Pdata = data, CLEAN = TRUE, verbose = TRUE)
	...
	data$Depth  = NA
	data$Sex    = data$SEX
	data$Length = data$lengthcm
	data$Weight = data$FISH_WEIGHT #Already in KGs
	data$Age    = data$age
	data$Fleet  = fleets
